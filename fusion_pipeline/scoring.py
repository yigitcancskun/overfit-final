...
def compute_message_scores(message_features: pd.DataFrame, refs: dict[str, Any], config: dict[str, Any]) -> pd.DataFrame:
    ...
    if "hashtag_count" in scores.columns:
        scores["message_score"] = scores["message_score"].where(
            ~pd.to_numeric(scores["hashtag_count"], errors="coerce").fillna(0).gt(5),
            1.0,
        )
    if "exclamation_count" in scores.columns:
        scores["message_score"] = scores["message_score"].where(
            ~pd.to_numeric(scores["exclamation_count"], errors="coerce").fillna(0).gt(10),
            1.0,
        )
...
def apply_final_score_weighting(df: pd.DataFrame, config: dict[str, Any]) -> pd.DataFrame:
    ...
    if "hashtag_count" in df.columns:
        df["final_score"] = df["final_score"].where(
            ~pd.to_numeric(df["hashtag_count"], errors="coerce").fillna(0).gt(5),
            1.0,
        )
    if "exclamation_count" in df.columns:
        df["final_score"] = df["final_score"].where(
            ~pd.to_numeric(df["exclamation_count"], errors="coerce").fillna(0).gt(10),
            1.0,
        )
    return df
